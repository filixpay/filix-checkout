...
IMAGE_NAME="${CHECKOUT_IMAGE_NAME:-filix-checkout}"
ENV_FILE="${ENV_FILE:-.env.local}"
PORT_HOST="${PORT_HOST:-3001}"
PORT_CONTAINER="${PORT_CONTAINER:-3001}"
# Production GHCR path: scripts/deploy-pull.sh sets SKIP_BUILD=1 and CANDIDATE_IMAGE.
...
if [ "${SKIP_BUILD}" = "1" ] && [ -z "${CANDIDATE_IMAGE:-}" ]; then
  echo "❌ 错误：SKIP_BUILD=1 需要设置 CANDIDATE_IMAGE（GHCR 镜像地址）。"
  exit 1
fi
